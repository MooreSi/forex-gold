...
def _insert_trade(trade_id, tg_source, strategy, signal_id=None):
...
def test_template_leg_inherits_channel_from_its_trade(fresh_db):
    ...
    src, strat = history._comment_attribution_maps({
...
def test_orphaned_sig_comment_recovers_channel_via_signal_id(fresh_db):
    ...
    src, strat = history._comment_attribution_maps({"900003": "sig:4ea5f031"})
...
def test_template_prefix_with_no_matching_trade_is_not_invented(fresh_db):
    ...
    src, _ = history._comment_attribution_maps({"900010": "ea:5b88a61e-6g1"})
    assert src == {}
