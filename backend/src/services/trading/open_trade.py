...
async def open_trade(
    ...
    # message's) resolved prices -- NOT the signal's own tp1..tp8, which is what
    # this row used to store while the EA ran something else entirely (found
    # 2026-07-30: a Sig Gen Grid trade alerted 8 Reversal Engine levels while the
    # EA ran the template's 6). A level the resolved ladder doesn't define is
    # NULL, so a 6-level template stops claiming 8.
    if _resolved_tps is not None:
        _row_tps = [_resolved_tps.get(n) for n in range(1, 9)]
    else:
        _row_tps = [tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8]

    # Realised-R inputs, captured now because neither survives the trade:
    # stop_loss is overwritten in place by every breakeven/trailing path, and
    # lot_size only describes one leg of a grid. This is the SEED -- profit_sync
    # replaces initial_risk with the figure from the legs that actually filled,
    # since a resting pending leg staged here may expire without ever taking on
    # risk. entry_price is 0 on a grid placeholder row, so fall back to the tick
    # the stop was measured from rather than booking a risk the width of the
    # whole gold price.
    ...
    trade_repo.insert_trade_and_activate_signal(
        ...
        *_row_tps,
        ...
        _grid_legs_total, stop_loss, round(_initial_risk, 4),
    )
