...
def test_close_full_no_residual_syncs_and_alerts(fresh_db):
    _insert_trade(mt5_ticket=555, remaining_lots=0.0)
    bridge = _FakeBridge(positions=[])
    with mock.patch.object(ps, "sync_profit", new=mock.AsyncMock(return_value=10.0)) as sp, \
         mock.patch.object(ps, "schedule_profit_sync", new=mock.AsyncMock()) as ssp:
        asyncio.run(ps.close_full_after_tps("t-1", 555, 2420.0, bridge))
    assert sp.called
    assert ssp.called
    assert _trade_dict()["status"] == "closed"


def test_close_full_residual_reopens_and_closes(fresh_db):
    _insert_trade(mt5_ticket=555, remaining_lots=0.0)
    bridge = _FakeBridge(
        positions=[{"ticket": 555, "volume": 0.02}],
        close_result={"success": True, "close_price": 2419.0},
    )
    with mock.patch.object(ps, "record_close", new=mock.AsyncMock()) as rc:
        asyncio.run(ps.close_full_after_tps("t-1", 555, 2420.0, bridge))
    trade = _trade_dict()
    assert trade["status"] == "open"
    assert trade["remaining_lots"] == 0.02
    rc.assert_called_once()
    call_args = rc.call_args.args
    assert call_args[0] == "t-1"
    assert call_args[1] == 2419.0
    assert call_args[2] == "all_tps_hit_residual"


def test_close_full_residual_close_fails_stays_reopened(fresh_db):
    _insert_trade(mt5_ticket=555, remaining_lots=0.0)
    bridge = _FakeBridge(
        positions=[{"ticket": 555, "volume": 0.02}],
...
def test_close_full_no_ticket_skips_profit_sync(fresh_db):
    _insert_trade(mt5_ticket=None, remaining_lots=0.0)
    with mock.patch.object(ps, "sync_profit", new=mock.AsyncMock()) as sp:
        asyncio.run(ps.close_full_after_tps("t-1", None, 2420.0, _FakeBridge()))
    assert not sp.called
