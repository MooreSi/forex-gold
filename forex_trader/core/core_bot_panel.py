...
* Numeric fields offer both -/+ steppers and a 'Set exact value' button. The
  ...
  '[field@slug]' token that _parse_prompt_token reads back off the user's
  reply, since callback data cannot round-trip through a text reply.
"""
...
_PROMPT_RE = re.compile(r"\[([A-Za-z0-9_]+)@([0-9a-f]{8})\]")
