"""_classify_vip_level used to hardcode "swing_high" as its fallback
whenever a VIP price didn't land within 4pts of the last cycle's top-8
proximity-filtered candidates -- which was most of the time, since those
candidates are filtered to levels near the BOT's own price, not the VIP's.
This silently mislabeled the majority of VIP-level pattern-learning data.
...
def test_exact_round_number_classified_without_any_cache():
    stub = _Stub({})
    assert stub._classify_vip_level(4000.0) == "round_10"
    assert stub._classify_vip_level(4005.0) == "round_5"


def test_zero_or_negative_price_is_unknown():
    stub = _Stub({})
    assert stub._classify_vip_level(0.0) == "unknown"
    assert stub._classify_vip_level(-5.0) == "unknown"


def test_close_match_in_last_cycle_cache_wins_first():
    stub = _Stub({"levels": [{"price": 4032.0, "type": "asia_low"}]})
    assert stub._classify_vip_level(4033.0) == "asia_low"


def test_falls_back_to_full_level_set_when_not_in_narrow_cache():
    """The narrow 'levels' cache misses (VIP price far from bot's own
    ...
    assert stub._classify_vip_level(4042.5) == "swing_low"
...
def test_no_nearby_level_anywhere_returns_unknown_not_swing_high():
    ...
    assert stub._classify_vip_level(4123.0) == "unknown"
