"""Proves backend.src.services.positions.handle_scalp_runner's extracted function
behaves identically to SimulationEngine._handle_scalp_runner,
characterized in test_handle_scalp_runner_characterization.py -- see
...
from backend.src.services.positions import handle_scalp_runner as hsr
...
def test_no_tp1_is_noop(fresh_db):
    ...
    bridge = _FakeBridge()
    asyncio.run(hsr.handle_scalp_runner(trade, _tick(bid=2405.0, ask=2405.5), bridge, TPCache()))
...
def test_tp1_not_cleared_is_noop(fresh_db):
    ...
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hsr.handle_scalp_runner(trade, _tick(bid=2401.0, ask=2401.5), bridge, TPCache()))
    assert bridge.partial_close_calls == []


def test_tp1_cleared_closes_50pct_sl_untouched(fresh_db):
    _insert_signal()
...
def test_tp1_auto_closed_returns_before_sl_move(fresh_db):
    _insert_signal()
...
def test_tp1_done_tp2_not_cleared_is_noop(fresh_db):
    _insert_signal()
    ...
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hsr.handle_scalp_runner(trade, _tick(bid=2403.5, ask=2404.0), bridge, TPCache()))
    assert bridge.partial_close_calls == []
    assert bridge.modify_order_calls == []
...
def test_phase3_trails_remaining_with_fixed_distance(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.05,
    ...
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hsr.handle_scalp_runner(trade, _tick(bid=2410.0, ask=2410.5), bridge, TPCache()))

    assert bridge.partial_close_calls == []
    assert bridge.modify_order_calls == [{"ticket": 555, "sl": 2407.0, "tp": None}]


def test_phase3_price_retreat_does_not_move_sl_backward(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.05,
