"""Characterizes _handle_conservative_trial on SimulationEngine
(core/engine.py) before task 020 extracts it -- see
docs/todo/refactor/core-conservative-trial-handler-migration/010-*.md.
...
def test_no_tp_cleared_is_noop(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
                  entry_price=2400.0, tp1=2405.0)
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative_trial(engine, trade, _tick(bid=2401.0, ask=2401.5)))
    assert engine._bridge.partial_close_calls == []


def test_tp1_alone_closes_5pct_falls_through_to_tp2_check(fresh_db, engine):
    # _handle_conservative_trial's own TP1 branch never touches stop_loss --
    # the docstring says SL only moves at TP2. But partial_close_trade
    # (pack 9) independently moves SL to breakeven whenever reason=="TP1"
    # (the same characterization finding recurring from packs 22/23), so
    # the DB row's SL reaches breakeven right at the 5% insurance close,
    # a full tier earlier than the handler's own code implies.
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
                  entry_price=2400.0, stop_loss=2390.0, tp1=2405.0, tp2=2410.0)
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative_trial(engine, trade, _tick(bid=2406.0, ask=2406.5)))

    assert engine._bridge.partial_close_calls == [{"ticket": 555, "lots": 0.005}]
    assert engine._bridge.modify_order_calls == []  # broker-side sync only comes from _move_sl, not TP1
...
def test_tp1_bridge_rejection_cascades_to_tp2_check_anyway(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
    ...
    # TP1 rejected -> no DB write, but _partial returning False means the
    # caller doesn't `return` -- it falls through to check TP2, which isn't
    # cleared at this price, so the call just ends.
    trade_after = _trade_dict("t-1")
    assert trade_after["remaining_lots"] == 0.10
    assert engine._bridge.modify_order_calls == []


def test_tp1_and_tp2_both_crossed_in_one_tick_cascades(fresh_db, engine):
    # TP1's partial_close_trade side effect (see the "falls_through" test
    # above) already writes stop_loss=2400.0/sl_moved_to_be=1 to the DB
    # before TP2 is even checked. But TP2's own SL-move guard reads
    # `current_sl`/`trade.get("sl_moved_to_be")` from the *in-memory* trade
    # dict captured at function entry (never refreshed mid-call) -- stale
    # values of 2390.0 / 0. So TP2 still thinks it needs to move SL, and
    # does: calling bridge.modify_order and rewriting stop_loss to the same
    # 2400.0 target (a harmless no-op overwrite) plus a second, duplicate
    # "SL moved" Telegram alert. Not a bug worth fixing during a verbatim
    # extraction -- just a real quirk to preserve and flag.
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
                  entry_price=2400.0, stop_loss=2390.0, tp1=2405.0, tp2=2410.0)
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative_trial(engine, trade, _tick(bid=2411.0, ask=2411.5)))
...
def test_tp2_alone_closes_30pct_moves_sl_to_be_unconditional_return(fresh_db, engine):
    _insert_signal()
    ...
    assert engine._bridge.modify_order_calls == [{"ticket": 555, "sl": 2400.0, "tp": None}]
    trade_after = _trade_dict("t-1")
    # Unconditional return means TP3 (also cleared at this price) is never checked.
    assert trade_after["stop_loss"] == 2400.0


def test_tp2_sl_already_moved_skips_move(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.095,
...
def test_tp3_alone_closes_20pct_falls_through_to_tp4_check(fresh_db, engine):
    _insert_signal()
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative_trial(engine, trade, _tick(bid=2415.0, ask=2415.5)))

    assert engine._bridge.partial_close_calls == [{"ticket": 555, "lots": 0.02}]
    trade_after = _trade_dict("t-1")
    assert trade_after["remaining_lots"] == pytest.approx(0.045)


def test_tp4_closes_40pct_steps_sl_to_tp2_level_unconditional_return(fresh_db, engine):
    _insert_signal()
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative_trial(engine, trade, _tick(bid=2421.0, ask=2421.5)))
...
def test_tp4_sl_step_guard_skips_move_when_tp2_not_beyond_current_sl(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.045,
...
def test_tp5_alone_closes_5pct_falls_through_to_tp6_check(fresh_db, engine):
    _insert_signal()
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative_trial(engine, trade, _tick(bid=2428.0, ask=2428.5)))
...
def test_tp6_closes_all_remaining_not_a_fixed_pct(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.005,
    ...
    for r in ("TP1", "TP2", "TP3", "TP4", "TP5"):
        _insert_partial_close("t-1", r)
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative_trial(engine, trade, _tick(bid=2436.0, ask=2436.5)))
...
def test_tp6_zero_remaining_is_noop(fresh_db, engine):
    _insert_signal()
