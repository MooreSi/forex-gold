...
- **Still not synced, found 2026-09-25:** (1) VPS-originated changes never reach the Mac. Every `_forward_*_over_sync` asks `client.get_instance()` first, which never returns None, so the server broadcast branch is unreachable on the VPS. (2) The strategy-parameter template library is per-node by design (`protocol.py`), so a channel strategy of `template:<name>` names a template the VPS may not have, and that channel's override stays pending on the Mac.
...
- **A database holding both "GD Copy Engine" and "Reversal Engine" rows crash-looped startup (2026-09-26, reported live on the VPS).** `backfills._rebrand_source_names` renames the old name on every boot, and three of its tables (`channel_performance`, `channel_parser_config`, `channel_strategy_rec`) are keyed by that name, so the rename hit `UNIQUE constraint failed` -- an `IntegrityError`, which `execute_tolerant`'s fail-closed policy rightly does not swallow. It is now `UPDATE OR IGNORE`, and where the new name was taken the old row is then deleted in those three config tables only (owner, same day: remove the duplicate; trade history renames cleanly and is not touched). **How it came to hold both (found later the same day):** the Mac's channel-strategy sync sends a `'GD Copy Engine'` entry (the Mac's own database still carries the old name), and the VPS applies it -- `[SyncServer] applied channel strategy from Mac: {'GD Copy Engine': ...}` in the VPS log at 11:40:02. It was not a row at all: it sat in the Mac's resend queue (`sync_pending_channel_strategy`) from before the rebrand and was re-sent on every reconnect. `_pending_store._load_pending_channel_strategy` now drops a `'GD Copy Engine'` entry and saves the cleaned queue -- dropped, not renamed, because its stale 'conservative' would overwrite Reversal Engine's real strategy. Pinned by `tests/core/test_pending_channel_strategy_drops_the_old_name.py`. Pinned by `tests/migrations/test_backfills.py::test_a_database_holding_both_names_still_starts`.
- **The Mac's MT5 accounts and its demo/live choice reach the VPS, which switches to match (owner, 2026-09-26).** Found that day: the VPS's bridge was set up for demo account *480 while its MetaTrader and the Mac were on *592, because nothing had ever carried the Mac's accounts across -- the settings sync deliberately never carries credentials. `sync/_mt5_accounts_sync.py` adds `MSG_MT5_ACCOUNTS` (Mac -> VPS): login, password and server for each account the Mac has COMPLETE, plus `environment`; never the terminal path, which is per machine. Sent on every connect, after every credentials save (`PUT /api/settings/mt5`) and after a demo/live switch, before the Mac restarts. The VPS stores them first (`environment.switch` refuses an account with no saved credentials), then: on another environment it runs the same guarded `environment.switch` and restarts (Telegram alert); on the same one it rewrites the bridge file and `align_bridge`s the running bridge. A refusal comes back in `MSG_MT5_ACCOUNTS_ACK` and is alerted on the Mac. **Owner's choice for a mismatch found on reconnect (e.g. the Mac switched while the VPS was offline): the VPS switches to match**, including onto live. Its open positions on the account it leaves keep their broker-held SL/TP but are no longer managed by it. Pinned by `tests/core/test_mt5_accounts_sync.py`. **The live half needs the owner's own session.**
- **Every restart path now comes back under the Windows launcher (2026-09-26).** The Restart button, `/restartapp` and the demo/live switch go through `telegram/bot_infra.cmd_restart_app`, which had its own relaunch: under the launcher it exited 0 ("stopped"), and its hidden relaunch never ran -- `restart.log`: "ERROR: Input redirection is not supported", Windows' `timeout` refusing to run without a console, with the relaunch chained on it by `&&`. It now calls `os_utils.ask_launcher_to_relaunch()` (exit 42) under the launcher, headless mode exits with the requested code, and `delayed_relaunch_cmd` waits with `ping -n N 127.0.0.1` instead of `timeout` for the launches that still need a detached relaunch. Pinned by `tests/utils/test_restart_under_the_launcher.py`.
