...
def main_page():
    ...
    # The three shell dialogs live in _dialogs.py. They are built HERE, before
    # the header, because a NiceGUI dialog renders where it is created and these
    # must sit at page root rather than inside the header row.
    from ._dialogs import (
        build_power_dialog, build_pause_dialog, build_resume_confirm_dialog,
    )

    _power_dialog = build_power_dialog(root)
    _pause_dialog = build_pause_dialog()
    _resume_confirm_dialog, _resume_confirm_reason_lbl = build_resume_confirm_dialog()
