...
def test_place_pending_order_wire_format():
    ...
    with pytest.raises(asyncio.TimeoutError):
        asyncio.run(bridge.place_pending_order(
            ...
            {1: 4151.0, 2: 4155.0, 3: 4160.0}, [0.25, 0.25, 0.25], 0,
...
def test_place_pending_order_raises_when_ea_unhealthy():
    ...
    with pytest.raises(ConnectionError):
        asyncio.run(bridge.place_pending_order(
            "trade-1", "BUY", 4148.0, 0.10, 4141.0, {1: 4151.0}, [1.0], 0,
        ))
...
def _insert_pending_order(trade_id="t1", signal_id="s1", tp_open=1):
    with db.db() as conn:
        conn.execute(
        ...
        conn.execute(
            ...
            (trade_id, signal_id, "tg1", "chan", "BUY", 4148.0, 4141.0,
             ...
             0, tp_open, 0.10, 999, "working", time.time()),
        )
