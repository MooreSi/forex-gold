"""The running build reports 6.1 "Stall fixes and a quieter log", from one
source, everywhere.

It replaces the 0.6 "React Frontend Migration" pin (owner, 2026-09-24) with
the same assertions: the release moved, so the pin moves with it.
...
class TestTheReleaseItself:
    def test_the_head_entry_is_6_1(self):
        assert vh.RELEASES[0][0] == "v6.1"

    def test_it_is_called_stall_fixes_and_a_quieter_log(self):
        assert vh.RELEASES[0][1] == "Stall fixes and a quieter log"
    ...
    def test_the_previous_release_is_still_there(self):
        ...
        assert vh.RELEASES[1][0] == "v0.6"
        assert vh.RELEASES[2][0] == "v0.5"
        assert len(vh.RELEASES) >= 13


class TestEveryReporterAgrees:
    def test_the_derived_version(self):
        assert vh.__version__ == "6.1"

    def test_the_settings_and_about_reporter(self):
        assert system_controller.app_version() == "6.1"
    ...
    def test_the_reporter_the_admin_console_reads(self):
        ...
        assert remote_controller.app_version() == "6.1"
    ...
    def test_the_VERSION_file_fallback(self):
        ...
        assert (REPO / "VERSION").read_text(encoding="utf-8").strip() == "6.1"
...
class TestTheChangelog:
    def test_it_has_a_6_1_entry(self):
        head = (REPO / "CHANGELOG.md").read_text(encoding="utf-8")[:400]

        assert "v6.1" in head
        assert "Stall fixes and a quieter log" in head
