...
The persistence goes through `settings_controller`, not `db.database`. This
module lived in `backend/src/utils/` while being pure UI, and moving it here
exposed the reason it had been filed as backend at all: two lines of it read
and wrote the database directly. Those two lines were the only thing making a
stylesheet a backend concern.
"""
...
def get_theme() -> str:
    stored = _settings.get_app_config(_THEME_KEY)
...
def set_theme(theme: str) -> None:
    ...
    _settings.set_app_config(_THEME_KEY, theme)
