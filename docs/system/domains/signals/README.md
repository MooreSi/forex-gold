...
- **That trigger matches per-LINE and exactly — not as a substring**, unlike every other lexicon here, and it refuses any message containing a digit. Both guards exist for the same reason: the shipped default list contains the bare word `BUY`, and as a substring that would open a market order on any message mentioning buying at all. Line-exact means `BUY` fires on a message whose line is `BUY`, and the no-digit rule keeps anything that states a level with the per-format parsers, which will use those levels rather than ignoring them. A message naming both directions is refused rather than guessed at.
