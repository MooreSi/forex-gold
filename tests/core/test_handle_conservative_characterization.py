"""Characterizes _handle_conservative on SimulationEngine (core/engine.py)
before task 020 extracts it -- see
docs/todo/refactor/core-conservative-handler-migration/010-*.md.
...
def test_no_tp1_is_noop(fresh_db, engine):
    ...
    asyncio.run(SimulationEngine._handle_conservative(engine, trade, _tick(bid=2405.0, ask=2405.5)))
...
def test_tp1_not_cleared_is_noop(fresh_db, engine):
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative(engine, trade, _tick(bid=2401.0, ask=2401.5)))
    assert engine._bridge.partial_close_calls == []


def test_tp1_cleared_closes_80pct_and_moves_sl_to_be(fresh_db, engine):
    _insert_signal()
...
def test_tp1_auto_closed_skips_modify_order_but_db_sl_still_moves(fresh_db, engine):
    # Tiny remaining -- 80% close empties the whole position, so
    # _handle_conservative's OWN "move SL to BE" block never runs (it
    # returns right after scheduling close_full_after_tps). But
    # partial_close_trade (pack 9) has its own independent BE-move baked
    # in whenever reason=="TP1" and move_sl_to_be_after_tp1 is on (the
    # default) -- that already wrote stop_loss=entry_price as part of
    # computing `res`, before _handle_conservative even checks
    # auto_closed. So the DB row's SL still ends up at breakeven, even
    # though bridge.modify_order (the broker-side sync, correctly
    # skipped -- there's no live position left to modify) never fires.
    _insert_signal()
...
def test_phase2_trails_remaining_20pct_with_fixed_distance(fresh_db, engine):
    _insert_signal()
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_conservative(engine, trade, _tick(bid=2410.0, ask=2410.5)))

    assert engine._bridge.partial_close_calls == []  # phase 2 never partial-closes
    assert engine._bridge.modify_order_calls == [{"ticket": 555, "sl": 2407.0, "tp": None}]  # bid-3


def test_phase2_price_retreat_does_not_move_sl_backward(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.02,
