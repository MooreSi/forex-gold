...
- The installer no longer bundles Python: it downloads the 3.11 embeddable runtime at install time, creates a venv under `%LOCALAPPDATA%\FOREX Trader\.venv\`, and adds firewall rules: 8888 and 9000 on the Private profile, 8765 (sync) on every profile (requires internet; the rules need admin).
...
- **A VPS must accept TCP 8765 (the sync server) and nothing else FOREX-specific (2026-09-25).** The original app left this as a manual step on its About page. The installer now adds `FOREX Trader Sync (port 8765)` inbound on **every** profile: a VPS's network is almost always Public, and the existing 8888/9000 rules are Private-only, so they open nothing there. Settings > Remote node asks `netsh ... show rule name=<that name>` (cached 60s; the tab polls every 3s) and shows the add command when it is missing, plus this machine's IPv4 addresses (outbound interface first) and a warning when they are all private, i.e. behind the provider's NAT. It never changes the firewall itself. `services/cluster/sync/reachability.py`; pinned by `tests/core/test_sync_reachability.py` and `tests/refactor/test_installer_prepares_a_vps.py`.
