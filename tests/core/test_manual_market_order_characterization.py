...
def test_raises_on_invalid_direction(fresh_db, engine):
    with pytest.raises(ValueError, match="Invalid direction"):
        asyncio.run(SimulationEngine.open_manual_market_order(engine, "SIDEWAYS", stop_loss=2390.0))
...
def test_raises_when_no_live_tick(fresh_db, engine):
    ...
    with pytest.raises(RuntimeError, match="No live price"):
        asyncio.run(SimulationEngine.open_manual_market_order(engine, "BUY", stop_loss=2390.0))


# ── SL resolution ──────────────────────────────────────────────────────────────

def test_explicit_sl_used_when_plausible(fresh_db, engine):
    result = asyncio.run(SimulationEngine.open_manual_market_order(engine, "BUY", stop_loss=2390.0))
...
def test_explicit_sl_too_close_raises(fresh_db, engine):
    with pytest.raises(ValueError, match="implausible"):
        asyncio.run(SimulationEngine.open_manual_market_order(engine, "BUY", stop_loss=2399.9))


def test_explicit_sl_too_far_raises(fresh_db, engine):
    with pytest.raises(ValueError, match="implausible"):
        asyncio.run(SimulationEngine.open_manual_market_order(engine, "BUY", stop_loss=1000.0))
...
def test_no_sl_dpm_disabled_raises(fresh_db, engine):
    ...
    with pytest.raises(ValueError, match="Stop Loss is required"):
        asyncio.run(SimulationEngine.open_manual_market_order(engine, "BUY"))


# ── lot sizing ─────────────────────────────────────────────────────────────────

def test_explicit_lot_size_used(fresh_db, engine):
    result = asyncio.run(SimulationEngine.open_manual_market_order(
