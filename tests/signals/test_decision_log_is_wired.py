...
class TestTheParsingPageCanReachIt:
    SOURCE = (REPO / "frontend/pages/telegram/_keywords.py").read_text(encoding="utf-8")
...
class TestTheReadoutIsReachable:
    ...
    SOURCE = (REPO / "frontend/pages/telegram/_decision_log.py").read_text(encoding="utf-8")
    PANEL = (REPO / "frontend/pages/telegram/__init__.py").read_text(encoding="utf-8")
    ...
    def test_the_page_renders_the_summary(self):
        assert "decision_log_summary" in self.SOURCE

    def test_the_page_renders_the_variant_report(self):
        assert "decision_log_report" in self.SOURCE

    def test_the_page_offers_the_backfill(self):
        assert "decision_log_backfill" in self.SOURCE
    ...
    def test_it_is_rendered_somewhere_a_user_can_reach(self):
        """A module nothing calls renders nothing."""
        assert "render_decision_log_section()" in self.PANEL
    ...
    def test_a_variant_with_no_evidence_is_not_rendered_as_zero(self):
        ...
        say so. Formatting it with %.3f would put 0.000 next to a variant
        that has never scored anything."""
        assert "is None" in self.SOURCE or "no decisions yet" in self.SOURCE
