"""The numbered migration steps themselves — the list, and nothing else.

Split out of `registry.py` 2026-09-10, a pure move. The list had grown to 661 of
that file's 800 lines and the next migration would have crossed the LOC ceiling
— a wall every future migration hits, rather than a problem with any one of
them. `registry.py` keeps the machinery that APPLIES them (apply_migration, run,
the schema-version stamp, the critical-schema check) and re-exports MIGRATIONS,
so `backend.migrations.registry.MIGRATIONS` and every other import path are
unchanged.

A peer module inside the existing `backend/migrations/` package, beside
`schema_sql.py` and `backfills.py` — the shape this package already had, not a
new flat sibling of the kind rule 70 warns about.

`_rename_gdc_column` came with the list rather than staying behind: it IS
migration step 1, and leaving it in registry.py would have made the two modules
import each other.

Rules for changing this file, unchanged from registry.py:
...
from backend.migrations.steps_recent import _RECENT
...
# Migrations 39 onward live in steps_recent.py: this file reached the
# 800-line ceiling at 38 and the registry's first rule is that a step is
# only ever appended. Splitting by number keeps that true -- nothing is
# renumbered, reordered or edited, the list is simply continued in a second
# file and concatenated here, so MIGRATIONS still reads 1..N in order.
MIGRATIONS += _RECENT
