"""Characterizes open_trade on SimulationEngine (core/engine.py) before
task 020 extracts it -- see docs/todo/refactor/core-open-trade-migration/010-*.md.

Uses a fake bridge test-double (get_fresh_tick/place_order) -- NO real or
demo MT5 order is ever placed, via the Python bridge OR the EA bridge, at
any point in this file. ea_bridge/sync.server/sync.client are exercised
via their own natural "unconfigured" defaults for the default-path tests,
and via explicit fakes (ea_bridge.set_instance, a patched sync.client.
get_instance) for the two non-default branches -- always reset in fixture
teardown since these are process-wide module-level singletons.
"""
import asyncio
import os
import tempfile
...
def fresh_db():
    ...
    _reset_db_worker_thread_connection()
    os.remove(path)
    ea_bridge.set_instance(None)  # never leak a fake EA into other test files
...
def test_raises_when_trading_paused(fresh_db, engine):
    _insert_signal()
...
def test_raises_when_max_open_trades_reached(fresh_db, engine):
...
def test_ea_managed_path_used_when_healthy_and_portable(fresh_db, engine):
    _insert_signal()
    ...
    assert engine._bridge.place_order_calls == []  # Python bridge never touched


def test_falls_through_to_python_bridge_when_no_ea_instance(fresh_db, engine):
    _insert_signal()
    db.update_risk_settings({"ea_bridge_enabled": 1})
...
def test_falls_through_to_python_bridge_when_ea_unhealthy(fresh_db, engine):
    _insert_signal()
    db.update_risk_settings({"ea_bridge_enabled": 1})
...
def test_falls_through_to_python_bridge_when_strategy_not_portable(fresh_db, engine):
    _insert_signal()
    db.update_risk_settings({"ea_bridge_enabled": 1})
...
def test_remote_forwarding_raises_stood_down_when_not_centralized(fresh_db, engine):
    _insert_signal()
...
def test_remote_forwarding_forwards_when_centralized(fresh_db, engine):
    _insert_signal()
