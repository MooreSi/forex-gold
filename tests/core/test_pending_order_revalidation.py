...
from forex_trader.core import core_pending_order_revalidation as por
...
def test_no_orders_makes_no_calls(fresh_db, monkeypatch):
    ea = _FakeEA()
    monkeypatch.setattr("forex_trader.core.ea_bridge.get_instance", lambda: ea)
    import asyncio
...
def test_order_within_grace_period_not_checked(fresh_db, monkeypatch):
    ...
    ea = _FakeEA()
    monkeypatch.setattr("forex_trader.core.ea_bridge.get_instance", lambda: ea)
    monkeypatch.setattr("forex_trader.core.dpm_engine.compute_atr", lambda *a, **k: 8.0)
    import asyncio
    asyncio.run(por.revalidate_pending_orders(_FakeBridge(_EXHAUSTED_SELL_CANDLE)))
    assert ea.cancelled == []
