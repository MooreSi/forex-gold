...
def statuses(monkeypatch):
    ...
    monkeypatch.setattr(
        "forex_trader.reversal_engine.reversal_engine_live_execute.re_db.update_live_exec",
        _update,
    )
    return seen
...
def live_enabled(monkeypatch):
    ...
    monkeypatch.setattr(
        "forex_trader.core.database.get_risk_settings",
        lambda: {"re_live_execution": 1, "strategy_lot_size": 0.01},
    )
    monkeypatch.setattr(
        "forex_trader.core.core_trading_schedule.check_trading_schedule",
        lambda **kw: (True, ""),
    )
...
def test_open_news_window_skips_live_execution(engine, statuses, live_enabled):
    with mock.patch(
        "forex_trader.core.news_calendar.check_news_blackout", return_value=_BLOCKED,
    ):
        _run(engine)
...
def test_clear_window_lets_the_flow_run_past_every_early_gate(engine, statuses, live_enabled):
    ...
    with mock.patch(
        "forex_trader.core.news_calendar.check_news_blackout", return_value=_CLEAR,
    ):
        _run(engine)
    assert statuses == {}
