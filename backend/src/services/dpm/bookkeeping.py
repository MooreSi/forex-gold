...
def load_dpm_calibrated(cache: DPMCache) -> dict:
    ...
    cal: dict = {}
    rows = dpm_repo.fetch_latest_calibration()
    for r in rows:
        key = f"{r[0]}_{r[1]}"
        cal[key] = {
            "be_multiplier":   r[2],
...
def record_dpm_entry(cache: DPMCache, trade: dict, params: dict) -> None:
    ...
    dpm_repo.insert_trade_performance(trade, params, time.time())
...
def finalize_dpm_record(trade_id: str, close_price: float,
    ...
    row = dpm_repo.get_trade_performance(trade_id)
    if not row:
        return
    ...
    dpm_repo.finalize_trade_performance(
        trade_id, close_price, exit_type, final_pnl,
        r_multiple, hold_minutes, time.time(),
    )
