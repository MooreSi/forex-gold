"""Characterizes _handle_no_sl_scale on SimulationEngine (core/engine.py)
before task 020 extracts it -- see
docs/todo/refactor/core-no-sl-scale-handler-migration/010-*.md.
...
def test_no_tps_defined_is_noop(fresh_db, engine):
    ...
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2406.0, ask=2406.5)))
    assert engine._bridge.partial_close_calls == []


def test_tp1_cleared_last_tp_closes_all(fresh_db, engine):
    _insert_signal()
...
def test_tp1_cleared_more_tps_exist_20pct_close_falls_through(fresh_db, engine):
    # _handle_no_sl_scale's own TP1 branch never touches stop_loss. But
    # partial_close_trade (pack 9) independently moves SL to breakeven
    # whenever reason=="TP1" (same recurring finding as packs 22/23/24) --
    # so the DB row's SL reaches breakeven right here too.
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
...
def test_tp1_behind_entry_market_at_be_partial_closes_at_market_price(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
                  entry_price=2400.0, stop_loss=2380.0,
                  tp1=2395.0, tp2=2410.0)  # TP1 behind fill price
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2400.0, ask=2400.5)))
...
def test_tp1_behind_entry_market_not_at_be_marks_skipped(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
                  entry_price=2400.0, stop_loss=2380.0,
                  tp1=2395.0, tp2=2410.0)
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2398.0, ask=2398.5)))
...
def test_tp2_cleared_last_tp_closes_all(fresh_db, engine):
    _insert_signal()
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2411.0, ask=2411.5)))

    assert engine._bridge.partial_close_calls == [{"ticket": 555, "lots": 0.08}]
    trade_after = _trade_dict("t-1")
    assert trade_after["status"] == "closed"


def test_tp2_cleared_more_tps_exist_marks_skipped_only(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.08,
...
def test_tp3_cleared_last_tp_closes_all(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.08,
                  entry_price=2400.0, stop_loss=2400.0,
                  tp1=2405.0, tp2=2410.0, tp3=2415.0)
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2416.0, ask=2416.5)))
...
def test_tp3_cleared_more_tps_exist_20pct_close_moves_sl_to_tp1(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.08,
                  entry_price=2400.0, stop_loss=2400.0,
    ...
    assert engine._bridge.partial_close_calls == [{"ticket": 555, "lots": 0.02}]
    assert engine._bridge.modify_order_calls == [{"ticket": 555, "sl": 2405.0, "tp": None}]
    trade_after = _trade_dict("t-1")
    assert trade_after["stop_loss"] == 2405.0
    assert trade_after["remaining_lots"] == pytest.approx(0.06)


def test_tp1_tp2_tp3_cascade_sl_move_uses_stale_current_sl(fresh_db, engine):
    # TP1's partial_close_trade side effect writes stop_loss=2400.0 (entry,
    # breakeven) to the DB. But TP3's own _move_sl compares its target
    # against `current_sl` -- a local float captured ONCE at function entry
    # (2380.0, the original emergency SL), never refreshed after TP1's
    # write earlier in this same call. So TP3 moves SL based on the stale
    # 2380.0 baseline, not the just-written 2400.0 -- a real, non-obvious
    # quirk (same category as the conservative_trial pack's stale-trade-dict
    # finding), preserved verbatim rather than "fixed".
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.10,
                  entry_price=2400.0, stop_loss=2380.0,
...
def test_tp4_not_last_steps_sl_to_tp2_level_marks_skipped(fresh_db, engine):
    _insert_signal()
    ...
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2421.0, ask=2421.5)))
...
def test_tp4_sl_step_guard_skips_move_when_not_beyond_current_sl(fresh_db, engine):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, lot_size=0.10, remaining_lots=0.06,
                  entry_price=2400.0, stop_loss=2412.0,  # already beyond tp2 (2410)
                  tp1=2405.0, tp2=2410.0, tp3=2415.0, tp4=2420.0, tp5=2425.0)
    for r in ("TP1", "TP2_SKIPPED", "TP3"):
        _insert_partial_close("t-1", r)
    trade = _trade_dict("t-1")
...
def test_last_defined_tp_within_loop_range_closes_all(fresh_db, engine):
    _insert_signal()
    ...
    for r in ("TP1", "TP2_SKIPPED", "TP3", "TP4_SKIPPED"):
        _insert_partial_close("t-1", r)
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2426.0, ask=2426.5)))
...
def test_tp8_final_branch_closes_all_when_max_tp_defined(fresh_db, engine):
    _insert_signal()
    ...
    for r in ("TP1", "TP2_SKIPPED", "TP3", "TP4_SKIPPED", "TP5_SKIPPED", "TP6_SKIPPED", "TP7_SKIPPED"):
        _insert_partial_close("t-1", r)
    trade = _trade_dict("t-1")
    asyncio.run(SimulationEngine._handle_no_sl_scale(engine, trade, _tick(bid=2441.0, ask=2441.5)))
