...
def _template_summary(tpl: dict) -> str:
    ...
    """
    from backend.src.services.broker.ea_templates import MAX_TP_LEVELS
...
def build_catalogue(include_hidden: bool = False) -> list[dict]:
    ...
    from backend.src.db import database as db_module
    from backend.src.utils.models import STRATEGY_DESCRIPTIONS, STRATEGY_NAMES
...
def resolve_key(raw: str | None, entries: list[dict]) -> str | None:
    ...
    """
    from backend.src.services.broker.ea_templates import TEMPLATE_OVERRIDE_PREFIX
