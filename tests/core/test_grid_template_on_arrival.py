...
from forex_trader.core import core_grid_template_dispatch as gtd
from forex_trader.core import core_pending_signal_activation as pa
...
def test_breakout_grid_zone_spans_the_retest(fresh_db):
    from forex_trader.breakout_signal.breakout_signal_live_execute import _grid_zone
...
@pytest.fixture
def bo_repo_db():
    from forex_trader.breakout_signal import breakout_signal_repo as bo_repo
    fd, path = tempfile.mkstemp(suffix=".db")
    os.close(fd)
    bo_repo.init(path)
    yield bo_repo
