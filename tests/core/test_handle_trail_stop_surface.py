...
class. NO real or demo MT5 order is ever modified -- verified via the
fake bridge's own call log.
"""
import asyncio
import os
import tempfile
...
def test_tp1_not_cleared_not_trailing_is_noop(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, tp1=2410.0)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hts.handle_trail_stop(trade, _tick(bid=2405.0, ask=2405.5), bridge, TPCache()))
    assert bridge.modify_order_calls == []


def test_tp1_cleared_activates_locks_sl_to_entry(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, entry_price=2400.0, stop_loss=2390.0, tp1=2410.0)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hts.handle_trail_stop(trade, _tick(bid=2412.0, ask=2412.5), bridge, TPCache()))
...
def test_stale_trade_dict_still_detects_active_trailing_via_db(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, entry_price=2400.0, stop_loss=2400.0, sl_moved_to_be=1)
    trade = _trade_dict("t-1")
    trade["sl_moved_to_be"] = 0
    bridge = _FakeBridge()
    asyncio.run(hts.handle_trail_stop(trade, _tick(bid=2408.0, ask=2408.5), bridge, TPCache()))
    assert bridge.modify_order_calls == [{"ticket": 555, "sl": 2403.0, "tp": None}]


def test_active_trailing_moves_sl_forward_with_price(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, entry_price=2400.0, stop_loss=2400.0, sl_moved_to_be=1)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hts.handle_trail_stop(trade, _tick(bid=2420.0, ask=2420.5), bridge, TPCache()))
    assert bridge.modify_order_calls == [{"ticket": 555, "sl": 2415.0, "tp": None}]


def test_active_trailing_never_moves_sl_backward(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, entry_price=2400.0, stop_loss=2415.0, sl_moved_to_be=1)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hts.handle_trail_stop(trade, _tick(bid=2410.0, ask=2410.5), bridge, TPCache()))
    assert bridge.modify_order_calls == []


def test_tp2_marker_recorded_once_while_trailing(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, entry_price=2400.0, stop_loss=2415.0, sl_moved_to_be=1,
                  tp2=2420.0)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    cache = TPCache()
    asyncio.run(hts.handle_trail_stop(trade, _tick(bid=2425.0, ask=2425.5), bridge, cache))
    ...
    asyncio.run(hts.handle_trail_stop(trade2, _tick(bid=2426.0, ask=2426.5), bridge, cache))
    with db.db() as conn:
