...
- One folder = one question: `NNN-short-slug/` with `run.py`, `README.md`,
  `RESULTS.md`, `output/`. Next number = highest existing + 1. Never rename
  or delete an existing experiment — negative results are assets.
...
- **`RESULTS.md` is machine-written, never hand-written**: every `run.py`
  ends with `report.write_results(...)` (`_shared/lib/report.py`), which
  emits the standard template — Headline (one plain-English sentence) /
  Numbers (the `metrics.summarize()` table) / Chart (equity curves PNG in
  `output/`) / Caveats / Verdict (`KEEP` | `DROP` | `NEEDS-M1` |
  `NEEDS-MORE-DATA` + one line). Re-running the experiment regenerates it;
  if RESULTS.md and run.py output disagree, re-run — never edit RESULTS.md
  by hand. README = the story; RESULTS.md = the standardised numbers.
- Charts only via `report.py` helpers (validated palette, one axis, max 4
  series). No ad-hoc matplotlib styling in experiment code.
...
2. README Result section filled in, dated, with the metric suite and the
   walk-forward (not just in-sample) numbers. `RESULTS.md` regenerated by
   that same run (never stale, never hand-edited).
