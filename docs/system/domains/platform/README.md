...
- The installer no longer bundles Python: it downloads the 3.11 embeddable runtime at install time, creates a venv under `%LOCALAPPDATA%\FOREX Trader\.venv\`, and adds firewall rules for 8888 and 9000 on the Private profile (requires internet; the rules need admin). It does not open 8765: "Make this node a VPS" does.
