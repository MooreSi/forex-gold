...
- **`db/database.py` must not import a service at module scope if that service imports `backend.src.db` back.** The re-export block is a layer inversion the refactor has not finished unwinding, and each eager entry closes a cycle: `sync_repo`, `channels/repo` and `analytics/read_repo` all import `backend.src.db`, whose `__init__` imports `database.py`. Whichever module the process imported first then decided whether it booted. `broker/credentials_repo` is a fourth, found on 2026-09-22 when this fix was finally rebased onto main: the test still failed on `environment_controller`, because the eager block for it had been added after the fix was written and sat unnoticed for three days. That is the argument for the test over the fix -- the shape keeps coming back, so the gate matters more than any one of these blocks. All four now resolve through `database.__getattr__` (`_SYNC_LAZY`, `_CHANNELS_LAZY`, `_SCORECARD_LAZY`, `_ANALYTICS_LAZY`, `_BROKER_CREDS_LAZY`) instead, which is the same idiom and costs nothing: `db_module.<name>` and `from ...database import <name>` both still work, and monkeypatching still binds a real global. Pinned by `tests/refactor/test_controllers_import_cold.py`, which cold-imports every controller in its own subprocess.
