"""Tests backend.src.services.analytics.orb_report's ORB methodology rebuilt
...
def test_auto_execute_not_proceeding_does_not_place_order(fresh_db):
    ...
    with patcher:
        asyncio.run(orb_exec.orb_auto_execute(_BULLISH_REPORT, _FakeBridge(), False))
    m.assert_not_called()
...
def test_auto_execute_direction_inside_does_not_place_order(fresh_db):
    patcher, m = _patch_open_market()
    with patcher:
        asyncio.run(orb_exec.orb_auto_execute({"direction": "inside"}, _FakeBridge(), True))
    m.assert_not_called()
...
def test_auto_execute_direction_unconfirmed_does_not_place_order(fresh_db):
    patcher, m = _patch_open_market()
    with patcher:
        asyncio.run(orb_exec.orb_auto_execute({"direction": "unconfirmed"}, _FakeBridge(), True))
    m.assert_not_called()
...
def test_auto_execute_missing_stop_or_target_does_not_place_order(fresh_db):
    patcher, m = _patch_open_market()
    with patcher:
        asyncio.run(orb_exec.orb_auto_execute({"direction": "bullish", "stop": None, "target": 2406.0},
                                          _FakeBridge(), True))
    m.assert_not_called()
...
def test_auto_execute_bearish_places_sell_market_order(fresh_db):
    patcher, m = _patch_open_market()
    with patcher:
        asyncio.run(orb_exec.orb_auto_execute(_BEARISH_REPORT, _FakeBridge(), True))
    args, kwargs = m.call_args
    assert args[1] == "SELL"
...
def test_auto_execute_market_order_failure_does_not_raise(fresh_db):
    ...
    with patcher, mock.patch("backend.src.services.telegram.alerts.send_message"):
        asyncio.run(orb_exec.orb_auto_execute(_BULLISH_REPORT, _FakeBridge(), True))  # must not raise
    m.assert_called_once()
