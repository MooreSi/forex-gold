...
shared DbAdapter (backend.src.db) instead of raw sqlite3 -- see
...
def get_all_signals(limit: int = 100) -> list[dict]:
    rows = get_db().all("SELECT * FROM test_signals ORDER BY created_at DESC, id DESC LIMIT ?", limit)
    return [_row(r) for r in rows]
