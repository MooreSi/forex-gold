...
def test_calculate_fees_default_settings(fresh_db):
    fees = TradingRuntime.calculate_fees(None, lot_size=0.10, spread=0.30, hold_hours=0.0)
...
def test_calculate_fees_swap_applies_after_24h(fresh_db):
    fees_short = TradingRuntime.calculate_fees(None, lot_size=0.10, spread=0.30, hold_hours=12.0)
    fees_long  = TradingRuntime.calculate_fees(None, lot_size=0.10, spread=0.30, hold_hours=48.0)
...
def test_pnl_buy_direction(fresh_db):
    result = TradingRuntime.pnl("BUY", entry=2400.0, current=2410.0, lots=0.10)
    assert result == round(10.0 * 0.10 * 100, 4)


def test_pnl_sell_direction(fresh_db):
    result = TradingRuntime.pnl("SELL", entry=2400.0, current=2390.0, lots=0.10)
    assert result == round(10.0 * 0.10 * 100, 4)  # SELL profits when price drops


def test_pnl_negative_for_adverse_move(fresh_db):
    result = TradingRuntime.pnl("BUY", entry=2400.0, current=2390.0, lots=0.10)
    assert result < 0


def test_suggest_lot_size_risk_based(fresh_db):
    lot = TradingRuntime.suggest_lot_size(
