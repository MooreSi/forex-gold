...
def test_excludes_instant_historical_rows(fresh_db):
    _insert_tg_signal("m-1", status="new")
    _insert_tg_signal("m-2", status="instant_historical")

    result = tgs.get_tg_signals()
    ids = [r["tg_message_id"] for r in result]
    assert ids == ["m-1"]


def test_orders_by_parsed_at_descending(fresh_db):
...
def test_respects_limit(fresh_db):
    for i in range(5):
        _insert_tg_signal(f"m-{i}", parsed_at=float(i))

    result = tgs.get_tg_signals(limit=2)
    assert len(result) == 2


def test_backfills_group_name_when_missing(fresh_db):
    reader = _FakeTgReader({"grp-1": "Gold Diggers VIP"})
    _insert_tg_signal("m-1", group_id="grp-1", group_name=None)

    result = tgs.get_tg_signals(tg_reader=reader)
...
def test_works_with_no_tg_reader(fresh_db):
    _insert_tg_signal("m-1", group_id="grp-1", group_name=None)
    result = tgs.get_tg_signals()
