"""Proves forex_trader.core.core_handle_orb_fixed's extracted function
behaves identically to SimulationEngine._handle_orb_fixed, characterized
in test_handle_orb_fixed_characterization.py -- see
...
class. NO real or demo MT5 order is ever closed -- verified via the fake
...
from forex_trader.core import core_handle_orb_fixed as hof
...
def test_no_tp1_hit_is_noop(fresh_db):
    _insert_signal()
    _insert_trade("t-1", tp1=2410.0)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hof.handle_orb_fixed(trade, _tick(bid=2405.0, ask=2405.5), bridge, TPCache()))
    assert bridge.partial_close_calls == []
    assert _trade_dict("t-1")["status"] == "open"


def test_no_remaining_lots_is_noop(fresh_db):
    _insert_signal()
    _insert_trade("t-1", tp1=2410.0, remaining_lots=0.0)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge()
    asyncio.run(hof.handle_orb_fixed(trade, _tick(bid=2415.0, ask=2415.5), bridge, TPCache()))
    assert bridge.partial_close_calls == []


def test_tp1_hit_with_mt5_ticket_closes_full_remaining_via_bridge(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, tp1=2410.0, remaining_lots=0.10)
    trade = _trade_dict("t-1")
    bridge = _FakeBridge(partial_close_result={"success": True, "close_price": 2411.5})
    asyncio.run(hof.handle_orb_fixed(trade, _tick(bid=2415.0, ask=2415.5), bridge, TPCache()))

    assert bridge.partial_close_calls == [{"ticket": 555, "lots": 0.10}]
    trade_after = _trade_dict("t-1")
    assert trade_after["status"] == "closed"
    assert trade_after["close_price"] == 2411.5


def test_bridge_rejection_aborts_without_db_write(fresh_db):
    _insert_signal()
    _insert_trade("t-1", mt5_ticket=555, tp1=2410.0, remaining_lots=0.10)
    trade = _trade_dict("t-1")
...
def test_no_mt5_ticket_skips_bridge_uses_signal_tp1_price(fresh_db):
    _insert_signal()
    ...
    trade_after = _trade_dict("t-1")
    assert trade_after["status"] == "closed"
    assert trade_after["close_price"] == 2410.0
